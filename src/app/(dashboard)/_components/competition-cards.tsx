...
export function CompetitionCards() {
	const { data: competitions, isLoading } =
		api.competition.getUserCompetitions.useQuery();
	...
	return (
		<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
			{competitions.map((competition) => {
				...
				return (
					<div className="rounded-lg border bg-card p-6" key={competition.id}>
						<div className="mb-4">
						...
						<div className="mb-4 space-y-2">
							...
							<div className="flex items-center justify-between text-sm">
								...
								<span className="font-medium">
									{competition._count.competitionUsers}
								</span>
							</div>
							...
							<div className="mb-4 rounded-md bg-red-50 p-3">
								<div className="flex items-center gap-2 text-red-800">
									...
									<span className="font-medium text-sm">
										{dashboardData.upcomingMatchesCount} match
										{dashboardData.upcomingMatchesCount !== 1 ? "es" : ""} need
										{dashboardData.upcomingMatchesCount === 1 ? "s" : ""}{" "}
										betting
									</span>
								</div>
							</div>
						...
						<Button
							asChild
							className="w-full"
							variant={hasUrgentMatches ? "default" : "outline"}
						>
