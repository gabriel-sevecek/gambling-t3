...
function BetResultCell({
	...
	bet: FinishedMatchWithBets["matchBets"][0] | undefined;
	actualResult: "HOME" | "DRAW" | "AWAY" | null;
}) {
	if (!bet) {
		return (
...
export function ResultsTable({ rows, currentUserId }: ResultsTableProps) {
	const matches = rows
		.filter(
			(row): row is { type: "match"; match: FinishedMatchWithBets } =>
				row.type === "match",
		)
		.map((row) => row.match);
			...
			<div className="sticky top-0 z-20 flex bg-background shadow-sm">
				...
				<div className="flex">
					{allUsers.map((user) => {
						...
						return (
							<div
								className={`flex w-8 shrink-0 flex-col items-center border-r border-b bg-background px-2 py-2 lg:w-12 lg:py-3`}
								key={user.id}
							>
			...
			<div className="divide-y">
				{rows.map((row, _index) => {
					if (row.type === "matchday") {
						return (
							<div
								className="flex bg-muted/50"
								key={`matchday-${row.matchday}`}
							>
